...
export const Item = ({
  ...
  return (
    <ItemComponent className={className} onClick={onClick} disabled={disabled}>
      {/* @ts-ignore */}
      {/* {IconComponent && <IconComponent className="mr-1.5 secondary-svg" />} */}
      {IconCustomComponent}
      {label}
    </ItemComponent>
  )
...
export const CategoryTitle = ({ children }: CategoryTitle) => {
  return (
    <div className="mt-4 first:mt-1.5 mb-1.5 text-[0.625rem] font-medium uppercase select-none px-1">
      {children}
    </div>
  )
}
...
export const Divider = forwardRef<HTMLHRElement>((props, ref) => {
  return <hr {...props} ref={ref} className="my-1 border-border" />
})
