...
const useContentItemActions = (
  ...
  const copyLinkToCurrentNode = useCallback(() => {
    if (currentNode) {
      const nodeId = currentNode.attrs.id
      if (nodeId) {
        // You might want to construct a full URL here, depending on your application's structure
        const link = `#${nodeId}`
        navigator.clipboard
          .writeText(link)
          .then(() => {
